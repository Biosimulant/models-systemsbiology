...
"""Tellurium-backed SBML BioModule wrapper for Zatorsky2006_p53_Model2."""
from __future__ import annotations

from biosim.contrib.sbml import TelluriumSBMLBioModule


class Zatorsky2006P53Model2Biomd0000000158Model(TelluriumSBMLBioModule):
    """Faithful SBML execution wrapper for Zatorsky2006_p53_Model2."""

    _SBML_ID = 'BIOMD0000000158'
    _TITLE = 'Zatorsky2006_p53_Model2'
    _TIME_UNIT = "model_time"
    _OBSERVABLE_STRATEGY = 'species'
    _OBSERVABLES = ['x', 'y', 'y0']
    _SPECIES_LABELS = {'x': 'p53', 'y': 'Mdm2', 'y0': 'precursor Mdm2'}
    _PARAMETER_INPUTS = {}
    _INITIAL_CONDITION_INPUTS = {'initial_precursor_mdm2': ('y0', 0.0, 'native SBML value', 'Source state initial condition exposed as a model-specific control because no explicit intervention parameter is identifiable. Maps to SBML symbol `y0`.'), 'initial_mdm2': ('y', 0.73, 'native SBML value', 'Source state initial condition exposed as a model-specific control because no explicit intervention parameter is identifiable. Maps to SBML symbol `y`.'), 'initial_model_state_p53': ('x', 0.28, 'native SBML value', 'Source state initial condition exposed as a model-specific control because no explicit intervention parameter is identifiable. Maps to SBML symbol `x`.')}
    ...
    def __init__(self, model_path: str = 'data/BIOMD0000000158.xml', integration_step: float = 0.1) -> None:
