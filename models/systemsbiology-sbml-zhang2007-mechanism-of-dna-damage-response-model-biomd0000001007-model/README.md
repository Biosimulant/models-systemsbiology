# Zhang2007 - Mechanism of DNA damage response (Model1)

**Source**: [biomodels_ebi](https://www.ebi.ac.uk/biomodels/BIOMD0000001007)
**Standard**: sbml
**Authors**: Tongli Zhang; Paul Brazhnik; John J Tyson

## Description

Its a mechanistic model explaining the impact of p53 om apoptosis decision. This model represents schema 1 of manuscript.


## Usage

This model was auto-generated from the BioModels EBI catalog.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/systemsbiology-sbml-zhang2007-mechanism-of-dna-damage-response-model-biomd0000001007-model
    alias: model
```

## Tags

systemsbiology, sbml, biomodels_ebi, auto-generated, biomodels-ebi, curated
