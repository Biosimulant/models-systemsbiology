# Zeilinger2006_PRR7-PRR9light-Y

**Source**: [biomodels_ebi](https://www.ebi.ac.uk/biomodels/BIOMD0000000096)
**Standard**: sbml
**Authors**: Melanie N Zeilinger; Eva M Farré; Stephanie R Taylor; Steve A Kay; Francis J Doyle

## Description

The model reproduces the time profile of cYm and cTm under light-dark cycles as depicted in Fig 4 and Fig 5 respectively. 12 hour light-dark cycles are accomplished using a simple algorithm in the eve


## Usage

This model was auto-generated from the BioModels EBI catalog.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/systemsbiology-sbml-zeilinger2006-prr7-prr9light-y-biomd0000000096-model
    alias: model
```

## Tags

systemsbiology, sbml, biomodels_ebi, auto-generated, biomodels-ebi, curated
